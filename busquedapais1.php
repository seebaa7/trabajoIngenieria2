...
<body background="imagenes/fondo4.jpg">
  <?php include("header.php"); ?>
   <section class="main container" >
        ...
        <div class="container">
            <h3>Resultados de Busqueda por Pais </h3>
        </div>      </section>
 ...
 <?php $pais = $_POST['pais'];
$datos = mysql_query("Select * from hospedaje where idp = '$pais' and borrado = 0");
$num = mysql_num_rows($datos);
if($num==0){
	echo"<script>alert('No hay hospedajes que cumplan los requisitos seleccionados.');window.location.href=\"busqueda.php\"</script>";
}		?>
<br>
<table style="color:white" class="table">
                ...
                <tbody>
                <?php while($dat=mysql_fetch_array($datos)){ $nombrepais = mysql_query ("Select * from paises where id = $pais  "); $nombrep = mysql_fetch_array($nombrepais) ;  $nombreprovincia = mysql_query ("Select * from provincia where idprov = $datos40[idprov]  "); $nombreprovincia1 = mysql_fetch_array($nombreprovincia)  ?>
                <tr >
				...
				<button class="btn" style="border:0;background:transparent;outline:none;-webkit-appearance:none" title="Detalles del Hospedaje"  type="submit"  name="id" value=<?php echo $dat['idh']; ?>> <span class="glyphicon glyphicon-search"></span></button>
				</form></a></td>
                </tr>
              <?php  }?> 
                      </tbody>
